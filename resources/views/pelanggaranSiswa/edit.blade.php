  ...
  <div class="col-lg-8 mx-auto">
    <div class="card-body">
      ...
      {{-- =========================
          ...
          method GET ke route edit => reload
      ========================= --}}
      @if(auth()->user()->hasAnyRole(['kesiswaan','guru_bk']))
        <form method="GET"
              action="{{ route('pelanggaranSiswa.edit', $pelanggaranSiswa->id) }}"
              class="row g-3 mb-2">
          <div class="col-md-8">
        ...
        <div class="alert alert-info py-2 mb-4">
          Pilih kelas (opsional). Halaman akan memuat daftar siswa pada kelas tersebut.
        </div>
      @endif
      ...
      ========================= --}}
      <form method="POST"
            action="{{ route('pelanggaranSiswa.update', $pelanggaranSiswa->id) }}"
            class="row g-3">
        @csrf
        @method('PUT')

        {{-- Untuk kesiswaan/guru_bk: kirim classroom_id untuk validasi pivot di store/update (opsional, tapi bagus) --}}
        @if(auth()->user()->hasAnyRole(['kesiswaan','guru_bk']))
        ...
        <div class="col-md-12">
          ...
          @else
            @php $currentId = old('siswa_id', $pelanggaranSiswa->siswa_id); @endphp

            <select name="siswa_id" class="form-select select2" data-placeholder="Pilih siswa..." required>
              <option value=""></option>
              @foreach($siswa as $s)
                <option value="{{ $s->id }}" @selected((int)$currentId === (int)$s->id)>
        ...
        <div class="col-md-4">
          ...
          <input type="date" name="tanggal_pelanggaran" class="form-control"
            value="{{ old('tanggal_pelanggaran', \Illuminate\Support\Carbon::parse($pelanggaranSiswa->tanggal_pelanggaran)->toDateString()) }}"
            required>
        ...
        @if(auth()->user()->hasAnyRole(['kesiswaan','guru_bk','wali_kelas']))
          @php
            $statusVal   = old('status', $pelanggaranSiswa->status);
            $tindakanVal = old('tindakan', $pelanggaranSiswa->tindakan);
          @endphp
          <div class="col-md-3">
          ...
          <select name="pelanggaran[]" class="form-select select2" multiple
            ...
            @foreach($pelanggaranByJenis as $jenis => $items)
              <optgroup label="{{ strtoupper($jenis) }}">
                @foreach($items as $p)
                  <option value="{{ $p->id }}" @selected(in_array($p->id, old('pelanggaran', $selectedIds)))>
                    {{ $p->nama }}
                  </option>
                @endforeach
              </optgroup>
        ...
        <div class="col-12">
          ...
          <textarea name="keterangan" class="form-control" rows="3">{{ old('keterangan', $pelanggaranSiswa->keterangan) }}</textarea>
        </div>
        ...
        @if(auth()->user()->hasRole('wali_kelas'))
          <div class="col-12">
            ...
            <textarea name="catatan_waliKelas" class="form-control" rows="3">{{ old('catatan_waliKelas', $pelanggaranSiswa->catatan_waliKelas) }}</textarea>
          </div>
        @elseif(auth()->user()->hasRole('kesiswaan'))
          <div class="col-12">
            <label class="form-label">Catatan Kesiswaan</label>
            <textarea name="catatan_kesiswaan" class="form-control" rows="3">{{ old('catatan_kesiswaan', $pelanggaranSiswa->catatan_kesiswaan) }}</textarea>
          </div>
        @elseif(auth()->user()->hasRole('guru_bk'))
          <div class="col-12">
            <label class="form-label">Catatan Guru BK</label>
            <textarea name="catatan_guruBK" class="form-control" rows="3">{{ old('catatan_guruBK', $pelanggaranSiswa->catatan_guruBK) }}</textarea>
          </div>
        @endif
